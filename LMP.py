...
class LMP:
    ...
    def __init__(self, name, cfg, fixed_vars, variable_vars, client, debug=False, env='omnigibson'):
        self._name = name
        self._cfg = cfg
        self._debug = debug
        ...
        self._context = None
        self.client = client
        self._cache = DiskCache(cache_dir= os.path.join(base_dir, "cache"), load_cache=self._cfg['load_cache'])
    ...
    def _cached_api_call(self, **kwargs):
        ...
        if kwargs['model'] != 'gpt-3.5-turbo-instruct' and \
            ...
            user2 = "I would like you to refine objects when the query involves open containers, such as beakers, dishes, erlenmeyer flasks, plastic bottles, trash bins, mugs, or similar items."
            assistant2 = 'Got it.'
            user3 = "I want you to refine the objects whenever you find extra requirements in the query, especially those that talk about how or where to grasp—no matter how they’re phrased (like 'make sure to grasp the narrow end', 'grasp the tip', or 'by the left end', etc.)—because they all refer to specific parts of the object."
            assistant3 = 'OK.'
            messages = [
